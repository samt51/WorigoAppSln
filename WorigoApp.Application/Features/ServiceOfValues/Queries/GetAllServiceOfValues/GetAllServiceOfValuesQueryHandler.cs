...
{
    public class GetAllServiceOfValuesQueryHandler : BaseHandler, IRequestHandler<GetAllServiceOfValuesQueryRequest, Response<IList<GetAllServiceOfValuesQueryResponse>>>
    {
        public GetAllServiceOfValuesQueryHandler(IMapper mapper, IUnitOfWork unitOfWork) : base(mapper, unitOfWork)
        {
        }

        public async Task<Response<IList<GetAllServiceOfValuesQueryResponse>>> Handle(GetAllServiceOfValuesQueryRequest request, CancellationToken cancellationToken)
        {
            ...
            if (maplist.Count == 0)
            {
                ...
                return new Response<IList<GetAllServiceOfValuesQueryResponse>>().Success(dataMap);
            }
            return new Response<IList<GetAllServiceOfValuesQueryResponse>>().Success(maplist);
        }
    }
}
